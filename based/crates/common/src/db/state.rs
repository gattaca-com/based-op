...
use revm::{
    ...
    db::{
        ...
        states::{
            BundleState, CacheAccount, TransitionAccount, TransitionState, bundle_state::BundleRetention,
            plain_account::PlainStorage,
        },
    },
};
use revm_interpreter::primitives::{
    Account, AccountInfo, Address, B256, BLOCK_HASH_HISTORY, Bytecode, HashMap, U256, db::Database, hash_map,
};
use revm_primitives::{Bytes, EvmState, address, b256, hex};
...
impl<Db> State<Db> {
    ...
    pub fn new_with_bundle(db: Db, bundle: BundleState) -> Self {
        State::builder().with_database(db).with_bundle_update().with_bundle_prestate(bundle).build()
    }
...
impl CacheState {
    ...
    pub fn new(has_state_clear: bool) -> Self {
        Self { accounts: HashMap::default(), contracts: HashMap::default(), has_state_clear }
    }
    ...
    pub fn trie_account(&self) -> impl IntoIterator<Item = (Address, &PlainAccount)> {
        self.accounts
            .iter()
            .filter_map(|(address, account)| account.account.as_ref().map(|plain_acc| (*address, plain_acc)))
    }
    ...
    fn apply_account_state(&mut self, address: &Address, account: &Account) -> Option<TransitionAccount> {
