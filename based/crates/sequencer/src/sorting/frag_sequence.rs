...
use alloy_consensus::{constants::KECCAK_EMPTY, proofs::ordered_trie_root_with_encoder, Header, EMPTY_OMMER_ROOT_HASH};
...
use revm::{db::states::bundle_state::BundleRetention, Database, DatabaseRef};
...
impl<Db: DatabaseRef> FragSequence<Db> {
    ...
    pub fn apply_sorted_frag(&mut self, in_sort: InSortFrag<Db>) -> FragV0 {
        FragV0::new(self.block_number, self.next_seq, in_sort.txs.iter().map(|tx| tx.tx.as_ref()), false)
    }
}
...
impl<Db> FragSequence<Db> {
        ...
        Db: DatabaseRead + Database<Error: Into<ProviderError> + Display>,
    {
        ...
        let (transactions, transactions_root, receipts_root, logs_bloom, gas_used) =
            self.seal_simulated_txs(canyon_active);

        let (state_root, _) = self.db.calculate_state_root(&bundle).unwrap();
        ...
        let header = Header {
            ...
            transactions_root,
            receipts_root,
            withdrawals_root: Some(KECCAK_EMPTY),
            logs_bloom,
        ...
        let v1 = ExecutionPayloadV1 {
            ...
            receipts_root,
            logs_bloom,
            prev_randao: block_env.prevrandao.unwrap(),
        ...
        tracing::info!("sealed block: {seal:#?}");
    ...
    /// Processes simulated txs and generates block data.
    /// Returns (tx_bytes, tx_root, receipts_root, logs_bloom, total_gas_used)
    #[inline]
    fn seal_simulated_txs(&self, canyon_active: bool) -> (Vec<Bytes>, B256, B256, Bloom, u64) {
        ...
        for t in self.txs.iter() {
            ...
            transactions.push(t.tx.envelope());
        }
        ...
        let transactions_root =
            ordered_trie_root_with_encoder(&transactions, |tx_bytes, buf| buf.extend_from_slice(tx_bytes));

        (transactions, transactions_root, receipts_root, logs_bloom, gas_used)
    }
}
...
mod tests {
    ...
    use crate::{block_sync::fetch_blocks::fetch_block, sorting::FragSequence, Simulator};
