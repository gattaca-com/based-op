...
pub(crate) async fn async_fetch_blocks_and_send_sequentially(
    ...
    while curr_block <= end_block {
        ...
        let mut blocks: Vec<Result<BlockWithSenders<Block<OpTransactionSigned>>, reqwest::Error>> = join_all(futures).await;
...
pub(crate) async fn fetch_block(block_number: u64, client: &Client, url: &str) -> Result<BlockWithSenders<Block<OpTransactionSigned>>, reqwest::Error> {
...
pub fn convert_block(block: RpcBlock<OpTxEnvelope>) -> BlockWithSenders<Block<OpTransactionSigned>> {
    ...
    let (converted_txs, senders): (Vec<_>, Vec<_>) = consensus_block.body.transactions.into_iter()
        .map(|tx| {
...
mod tests {
    ...
    async fn test_single_block_fetch() {
        ...
        assert_eq!(block.header.hash_slow().to_string(), "0xad9e6c25e60e711e5e99684892848adc06d44b1cc0e5056b06fcead6c7eb6186");
