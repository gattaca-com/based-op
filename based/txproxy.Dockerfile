...
COPY . .
RUN --mount=from=reth,target=/reth cargo build --release --bin based-txproxy
...
COPY --from=builder /app/target/release/based-txproxy /usr/local/bin
ENTRYPOINT ["/usr/local/bin/based-txproxy"]
