...
fn main() {
    ...
    let db_bop =
        init_database(args.db_datadir.clone(), args.max_cached_accounts, args.max_cached_storages).expect("can't run");
    ...
    std::thread::scope(|s| {
        ...
        s.spawn(|| {
            let sequencer = Sequencer::new(db_bop, db_frag.clone(), rt, SequencerConfig::default());
